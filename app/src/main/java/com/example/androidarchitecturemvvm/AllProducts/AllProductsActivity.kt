...
class AllProductsActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this, AllProductsFactory(repo)).get(AllProductsViewModel::class.java)
            ...
            if (products.isNullOrEmpty()) {
                ...
                LazyColumn {
                    items(products!!) { product ->
                        ProductsRow(product = product, action = { isOnline ->
    ...
    @Composable
    fun ProductsRow(product: Products, action: (isOnline: Boolean) -> Unit) {
        Row(
            ...
            Column(
                ...
                Button(onClick = { viewModel.addToFav(product) }) {
